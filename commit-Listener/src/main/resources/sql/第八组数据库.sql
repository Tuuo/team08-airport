...
CREATE TABLE TEAM08.DFOE_DFIE(
	...
	MIST VARCHAR,--内部航班状态2，国内、国际、地区航班不使用该内容，当航班属性为混合时特指国际部分状态
	...
	MCDP VARCHAR,--值机柜台及值机柜台区域显示2，国内、国际、地区航班不使用该内容，当航班属性为混合时特指国际旅客显示（按照集合的方式显示值机柜台）
	CHLS VARCHAR,--航班行李滑槽动态更新信息标签
	STLS VARCHAR,--航班机位动态更新信息标签
	NMCD VARCHAR,--国内航站楼编号
	JMCD VARCHAR,--国际航站楼编号
	FLDT VARCHAR,--第一件行李上架时间
	LLDT VARCHAR,--最后一件行李上架时间
	CONT VARCHAR,--起降架次
	PROX VARCHAR--航班代理
);
...
CREATE TABLE TEAM08.DFOE_DFIE_ARPT(
	...
	APNO VARCHAR,--航站序号
	APCD VARCHAR,--航站三字码
	FPTT VARCHAR,--计划起飞时间，如果有<DFLT>，如果航班为出港航班，第一航站（始发站）计划起飞时间必须有值
	FETT VARCHAR,--预计起飞时间，如果有<DFLT>
	FRTT VARCHAR,--实际起飞时间，如果有<DFLT>
	FPLT VARCHAR,--计划降落时间，如果有<DFLT>
	FELT VARCHAR,--预计降落时间，如果有<DFLT>
	FRLT VARCHAR,--实际降落时间，如果有<DFLT>
	APAT VARCHAR,--航展属性，（2403，2401，2404，2402），2403国内，2401国际，2404混合，2402地区
...
CREATE TABLE TEAM08.DFOE_DFDL(
	...
	DRLT VARCHAR,--备降实际降落时间，如果有<APOT>
	VIP VARCHAR,--VIP人数，如果该标签值为0，表示无VIP，大于0则表示有VIP，由于机场业务操作原因，可能导致该标签表示VIP人数不准确
	SFAW VARCHAR,--共享航空二字码，如果有<SFLT>
	SFNO VARCHAR,--共享航班号，如果有<SFLT>
	PAST VARCHAR,--前站实际起飞时间
	GTLS VARCHAR,--航班登机门动态信息标签
	BLLS VARCHAR,--航班行李提取转盘动态信息标签
	FCES VARCHAR,--航班旅客值机预计开始时间1，当航班属性为国内、国际、地区时使用，当航班属性为混合时特指国内旅客值机时间
...
CREATE TABLE TEAM08.DFOE_DFDL_ARPT(
	...
	FELT VARCHAR,--预计降落时间，如果有<DFLT>
	FRLT VARCHAR,--实际降落时间，如果有<DFLT>
	APAT VARCHAR,--航站属性，（2403，2401，2404，2402），2403国内，2401国际，2404混合，2402地区
	DFLT_ID number(6)
);
